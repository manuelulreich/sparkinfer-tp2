...
1. **How do we identify ALL changes** that make sparkinfer run on two RTX 5060 Ti cards with tensor-parallel `tp=2` over P2P? Dependency policy (as of the 2026-09-25 user decision): **one** C runtime dependency allowed — NVIDIA NCCL for the collective ops — with the hand-rolled 2-node design kept as the recorded fallback; the no-Python-stack promise holds and the 2.5 MB promise is *re-stated* as binary + libnccl (it was: no NCCL, no new dependencies at all).
...
It is **not** the implementation plan. It defines no code, no APIs, and no patch order. The record it produces is the append-only change manifest (`02-change-manifest.json`), currently **3,115 items**: 34 hand-audited (CHG-0001…0034) + 3,081 mechanical scan candidates from `scan.py` (re-scanned against the post-`20a4fbd` tree).
...
|---|---|---|---|---|
| **R1** (top) | **No CUDA peer access at all** on the consumer pair (no NVLink; platform-dependent: BIOS, PCIe routing, ACS). Then every all-reduce stages through host memory: the bytes cross PCIe at ≈ 25 GB/s effective vs 448 GB/s GDDR7. **Downgraded 2026-09-25: this is a performance fact, not a blocker** — NCCL's SHM transport works without P2P, so tp=2 is correct either way; only a *comm-init* failure is fatal, and that is R8. | P0 probe: `cudaDeviceCanAccessPeer` both ways + the NCCL smoke (which transport it picks, at what measured bandwidth). | The comm wrapper (NCCL) selects the transport; the probe records the live transport and its cost for the perf model. If the NCCL route is dead (R8), the hand-rolled 2-node design behind the same API (CHG-0005 fallback) restores the original no-deps promise; `tp=2` is refused at startup with a clear diagnostic only if *both* routes fail. | CHG-0027, 0006, 0005 |
...
| **R7** | **P2P exists but is slow.** Path routed through the root complex (ACS/IOMMU), or PCIe-generation-limited: effective bandwidth far below link spec, so per-layer all-reduce cost may matter on 48-SM cards even though the messages are tiny. | P0: measured D2D (peer and staged) vs GDDR7/local-link reference; `nvidia-smi topo -m` recorded; NCCL's own transport report. | Set performance expectations from **measured** P0 numbers, not datasheet numbers; the perf model in the next step uses the P0 table. The comm wrapper's transport selection (R1) already covers the cost case. | CHG-0006, 0027, 0005 |
| **R8** (new 2026-09-25) | **NCCL itself may not run on this pair.** The dependency decision (NCCL allowed, CHG-0032) rests on NCCL's prebuilds supporting consumer sm_120 and initialising a 2-rank communicator on a PCIe-only, no-NVLink consumer box — historically untested territory (NCCL's prebuilds have been datacenter-oriented). | The P0 probe's NCCL smoke, on the target box, day one: version + `ncclCommInitRank` + one all-reduce, with `NCCL_DEBUG=INFO` transport report. | If it fails: the `gpu_link` wrapper (CHG-0005) switches to the hand-rolled 2-node design behind the identical API — zero rework upstream, original no-deps promise restored, and every G1/R1/R7 answer stays valid. The decision cannot be killed by this risk; at worst it falls back to the original design. | CHG-0005, 0032, 0006 |
...
- **Comms dependency (revised 2026-09-25):** the 2-GPU all-reduce is **NCCL** behind a thin wrapper (`gpu_link`) — the user relaxed the original no-NCCL rule because NCCL is NVIDIA's optimized collective library, and its transport selection (P2P vs SHM host-staging) absorbs the R1 branch. Fallback if NCCL cannot run on this consumer pair (R8): the original hand-rolled design — two peer copies + a local add, P2P-mapped or pinned-staging behind one API, which is all a 2-GPU ring ever needs — restoring the 2.5 MB / no-Python-stack envelope (CHG-0005, CHG-0032).
